    ...
    layerEl.innerHTML = `
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-2">
          <button data-action="drag-handle" data-index="${index}" class="p-0.5 text-zinc-500 hover:text-zinc-300 cursor-grab active:cursor-grabbing rounded transition" title="拖拽排序" draggable="true">
      ...
      ${showCropControls ? `
        <div class="pt-2 border-t border-zinc-800/50 flex flex-col space-y-2">
          ...
          ${isCustom ? `
            <div class="space-y-2 bg-amber-500/5 p-2.5 rounded border border-amber-500/10">
              <div class="flex items-center justify-between pb-1.5 mb-1 border-b border-amber-500/10">
                <span class="text-[10px] text-amber-400/80 font-semibold tracking-wider">独立裁剪微调</span>
  ...
  layers.forEach(layerEl => {
    ...
    layerEl.addEventListener('dragover', (e) => {
      ...
      if (targetIndex !== dragSourceIndex) {
        layerEl.classList.add('layer-drag-over');
      }
    });

    layerEl.addEventListener('dragleave', () => {
      layerEl.classList.remove('layer-drag-over');
    });
    ...
    layerEl.addEventListener('drop', (e) => {
      ...
      if (dragSourceIndex !== null && dragSourceIndex !== targetIndex) {
        ...
        imgList.splice(targetIndex, 0, moved);
        updateLayersList();
        renderCollage();
...
function renderCollage() {
  ...
  if (imgList.length === 0) {
    preview.innerHTML = `
      <div class="py-24 text-center text-zinc-600 flex flex-col items-center justify-center space-y-3">
        ...
        <span class="text-xs">预览区为空，请在左侧上传影视截图</span>
      </div>
    `;
  ...
  imgList.forEach((item, index) => {
    ...
    if (index < imgList.length - 1 && globalSettings.gapSize > 0) {
      const scaledGap = globalSettings.gapSize * scaleFactor;
      block.style.marginBottom = `${scaledGap}px`;
    }
  ...
  // 估算真实导出的高清像素尺寸
  let actualExportHeight = 0;
  imgList.forEach((item, index) => {
    const aspect = item.aspectRatio || (16 / 9);
    const finalH = item.customCrop ? item.hPercent : globalSettings.hPercent;
    if (item.mode === 'full') {
      actualExportHeight += baseWidth / aspect;
    } else {
      actualExportHeight += baseWidth * (finalH / 100);
    }
    if (index < imgList.length - 1) {
      actualExportHeight += globalSettings.gapSize;
    }
  });

  document.getElementById('previewResolution').innerText = `${baseWidth} x ${Math.round(actualExportHeight)} px`;
}
...
document.getElementById('globalYPercentSlider').addEventListener('input', () => {
  debouncedRenderCollage();
  // 全局滑块变化时也需要更新图层列表中显示的全局参数值
  updateLayersList();
});
document.getElementById('globalHPercentSlider').addEventListener('input', () => {
  debouncedRenderCollage();
  updateLayersList();
});
